...
class QRDecoderService:
    """Service for decoding QR codes from images using OpenCV QR detector"""
    
    @staticmethod
    def decode_image(image_data: bytes) -> Optional[str]:
        """Decode QR code from image bytes using OpenCV QR detector"""
        try:
            # Validate input
            ...
            if data and len(data) > 0:
                logger.info(f"QR Code detected successfully")
                return data
            
            # If not found, try with grayscale
            gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
            data, bbox, _ = qr_detector.detectAndDecode(gray)
            
            if data and len(data) > 0:
                logger.info(f"QR Code detected in grayscale image")
                return data
            ...
            for method in [
                ...
                try:
                    ...
                    if data and len(data) > 0:
                        logger.info(f"QR Code detected with preprocessing")
                        return data
                except Exception as e:
                    logger.debug(f"Preprocessing method failed: {str(e)}")
                    continue
